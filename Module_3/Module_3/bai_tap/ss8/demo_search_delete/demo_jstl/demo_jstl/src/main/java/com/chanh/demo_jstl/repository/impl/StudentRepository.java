...
public class StudentRepository implements IStudentRepository {
    ...
    private final String SEARCH ="SELECT s.*, c.name as class_name FROM student s join class c on s.class_id=c.id where s.name like  concat('%',?,'%') and c.name like  concat('%',?,'%');";
    ...
    @Override
    public List<StudentDto> search(String searchName, String searchClassName) {
        List<StudentDto> studentList = new ArrayList<>();
        // kết nối DB
        Connection connection = ConnectDB.getConnectDB();
        try {
            ...
            preparedStatement.setString(2,searchClassName);
            ResultSet resultSet = preparedStatement.executeQuery();
            while (resultSet.next()){
                int id =  resultSet.getInt("id");
                String name = resultSet.getString("name");
